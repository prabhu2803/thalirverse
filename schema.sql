...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  region VARCHAR(50),
  security_question TEXT,        -- for self-service password reset (no reachable email/phone)
  security_answer_hash TEXT,     -- sha256 hex, set via set_security_answer() — never stored plaintext
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
-- =======================================================
-- Self-service password reset via security question (student/admin
-- accounts have no reachable real email/phone for a reset-link flow).
-- =======================================================
...
-- Public lookup (unauthenticated — the forgot-password flow): returns the
-- security question text for a name, only when it resolves to exactly one
-- account that has one set. Never returns the answer.
...
-- Public verification (unauthenticated): returns the matching profile's id
-- only when the name resolves uniquely AND the hashed answer matches. The
-- server route uses this id + the service role key to set the new
-- password — this function never touches auth.users itself.
